...
func TestErrorHandling(t *testing.T) {
	tests := []struct {
		...
		{
			`{"name": "Monkey"}[fn(x) { x }];`,
			"FUNCTION is unusuable as a Hash Key!",
		},
	}
...
func TestHashLiterals(t *testing.T) {
	...
	if !ok {
		t.Fatalf("Evaluate did not return an object.Hash. Instead received a '%T'(%+v)", evaluated, evaluated)
	}
	...
	if len(result.Pairs) != len(expected) {
		t.Fatalf("Incorrect amount of Hash elements detected! Expected %d but receieved '%d'", len(expected), len(result.Pairs))
	}
